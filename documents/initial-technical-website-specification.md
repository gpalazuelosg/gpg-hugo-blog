...
A technical website featuring public content (blog, reviews, affiliate links) with private content management capabilities through a headless CMS. The site will be built using Hugo as the static site generator with a headless CMS backend with security (suggest me: either Strapi or Sanity as the backend CMS).

Our main goal is to create an MVP in less than one week, including the deployment infrastructure, CI/CD with staging site and supporting the authentication for the site owner.
